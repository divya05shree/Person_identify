...
from Person_identify.Activity_Monitor.sleep_detection import SleepDetector
...
class FaceRecognitionApp:
    def __init__(self, root):
        ...
        self.sleepDetection = SleepDetector()
        self.activityStatus = {}
        self.inactivity_timers = {}  # Track inactivity time for each person
        self.inactivity_threshold = 10  # 10 seconds threshold
        # Model setup
    ...
    def process_frames(self):
        ...
        while self.is_recognizing:
            try:
                ...
                # Detect faces
                face_crops = self.detect_face(detection_frame)

                # Scale coordinates back to original frame size
                scale_x = width / self.detection_size[ 0 ]
                scale_y = height / self.detection_size[ 1 ]
                        ...
                        if len(embeddings) > 0:
                            ...
                            for i, ((x, y, w, h), dist, idx) in enumerate(zip(face_coords, distances, indices)):
                                ...
                                if dist[ 0 ] < 0.8:
                                    ...
                                    activityStatus = self.sleepDetection.detect_sleep(face_rgb)

                                    # Update activity status
                                    if self.activityStatus.get(name):
                                        self.activityStatus[ name ].append(activityStatus)
                                    else:
                                        self.activityStatus[ name ] = [ activityStatus ]

                                    # Check for inactivity
                                    if not activityStatus:  # If person is inactive
                                        if name not in self.inactivity_timers:
                                            self.inactivity_timers[ name ] = time.time()
                                        elif time.time() - self.inactivity_timers[ name ] >= self.inactivity_threshold:
                                            # Show warning popup
                                            print("Stoppppppeddd in active")
                                            exit()
                                            self.root.after(0, lambda n=name: self.show_inactivity_warning(n))
                                            self.inactivity_timers[ name ] = time.time()  # Reset timer
                                    else:
                                        # Reset timer if person is active
                                        self.inactivity_timers.pop(name, None)
    ...
    def show_inactivity_warning(self, name):
        """Show a warning popup for inactive person"""
        messagebox.showwarning(
            "Inactivity Warning",
            f"{name} has been inactive for {self.inactivity_threshold} seconds!"
        )
